...
def main():
    ...
    pygame.display.set_caption('Run The Jewels')
...
def runGame():
    ...
    # gets current high score from a text file and assigns to variable
    ...
    while True:  # main game loop
        ...
        if clickedSpace and not firstSelectedGem:
            ...
            if matchedGems == []:
                ...
                # checks if current score is greater than the high score currently - read from file at start of game,
                # updated after. If greater, overwrites in file and updates variable if score increases in game
        ...
        if gameIsOver:
            if clickContinueTextSurf == None:
                if highScoreSet == False: # checks if a high score has been set in the game - if so, game over text will
                                          #
                    ...
                    clickContinueTextSurf = BASICFONT.render('High Score: %s (Click to continue)' % (score), 1,
                                                             GAMEOVERCOLOR, GAMEOVERBGCOLOR)
            ...
            lastScoreDeduction = time.time()
        drawScore(score)
        drawHighScore(currentHighScore)
...
def drawScore(score):
    ...
    scoreRect.bottomleft = (80, WINDOWHEIGHT - 6)
    DISPLAYSURF.blit(scoreImg, scoreRect)


def drawHighScore(currentHighScore):
    scoreImg = BASICFONT.render("High Score: " + str(currentHighScore), 1, SCORECOLOR)
    scoreRect = scoreImg.get_rect()
    scoreRect.bottomleft = (300, WINDOWHEIGHT - 6)
    DISPLAYSURF.blit(scoreImg, scoreRect)
